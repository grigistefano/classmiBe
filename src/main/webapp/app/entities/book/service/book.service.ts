...
type RestOf<T extends IBook | NewBook> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class BookService {
  ...
  protected convertDateFromClient<T extends IBook | NewBook | PartialUpdateBook>(book: T): RestOf<T> {
    return {
      ...book,
      createdDate: book.createdDate?.toJSON() ?? null,
      lastModifiedDate: book.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBook: RestBook): IBook {
    return {
      ...
      lastModifiedDate: restBook.lastModifiedDate ? dayjs(restBook.lastModifiedDate) : undefined,
    };
  }
